...
export const ChatMessage = ({ message, onQuickReply }: ChatMessageProps) => {
  const { id, sender, type, content, timestamp, imageSrc: initialImageSrc, audioSrc, audioDuration, status, options, beforeImageSrc, afterImageSrc, autoplay, playbackDelay } = message;
        ...
        return (
          <div className={cn('w-full flex items-center gap-2 text-sm p-2 rounded-lg', isUser ? 'bg-teal-500 text-white' : 'bg-gray-200 dark:bg-gray-700')}>
            ...
            <div className="flex-grow">
              <AudioPlayer src={audioSrc} duration={audioDuration} autoplay={autoplay} id={`audio-${id}`} playbackDelay={playbackDelay} />
